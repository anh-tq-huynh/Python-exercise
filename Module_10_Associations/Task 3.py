    ...
    def fire_alarm(self):
        ...
        for i in self.ele_list:
            if i.bot_floor != 1:
                while i.bot_floor > 1:
                    i.floor_down()
                print(f"Elevator {self.ele_list.index(i) + 1}/{self.total_ele} has successfully arrived floor 1")
            else:
                print(f"Elevator {self.ele_list.index(i) + 1}/{self.total_ele} is already at floor 1")
build = Building(12,5)
print(f"The building has a total of {build.total_ele} elevators and {build.top_floor} floors")
build.run_elevator(2,7)
build.run_elevator(3,4)
build.fire_alarm()